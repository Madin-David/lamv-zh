...
/* 0D integrals */
label("\fbox{0D 积分}", (1,9.5), deepblue);
string s = "在点处求 $f$ 的值";
...
/* 1D integrals */
label("\fbox{1D 积分}", (4,9.5), deepblue);
...
label("18.01 积分", t*(dir(90)/2), deepgreen);
...
/* 2D integrals */
label("\fbox{2D 积分}", (8,9.5), deepblue);
...
label("二重/面积积分", t*dir(90), deepgreen);
add(t*axis2);
...
label("曲面积分", t*dir(90), deepgreen);
add(t*axis3);

/* 3D integrals */
label("\fbox{3D 积分}", (12.5,9.5), deepblue);
...
label("三重/体积积分", t*dir(90), deepgreen);
add(t*axis3);
...
add(t*axis3);
add(t*vector_field);
label("通量", t*dir(90), purple);

/* Arrows */
draw("$\frac{df}{dx}$", (2,8)--(3,8), red, EndArrow);

draw((1,5)--(1,5.5)--(3,5.5), red, EndArrow);
label("$\nabla f$ (梯度)", (1,5.5), dir(180), red);
draw((5,5.5)--(8,5.5)--(8,5), red, EndArrow);
label("$\pdv{q}{x}-\pdv{p}{y}$ (2D 标量旋度)", (8,5.5), dir(0), red);

draw((1,-1.5)--(1,-3)--(3,-3), red, EndArrow);
label("$\nabla f$ (梯度)", (1,-2.25), dir(180), red);
draw((5,-3)--(7,-3), red, EndArrow);
label("$\nabla \times \mathbf{F}$ (旋度)", (6,-3), dir(270), red);
draw((9,-3)--(12.5,-3)--(12.5,-1.5), red, EndArrow);
label("$\nabla \cdot \mathbf{F}$ (散度)", (12.5,-2.25), dir(0), red);

/* Stokes */
label("$\bullet$ 任意相邻两个{\color{red}红色箭头}复合为零。", (6.0,7.7), dir(0), fontsize(9pt));
label("$\bullet$ 若 {\color{red}$X \longrightarrow Y$}，则
